...
class HargassnerBridge:
    ...
    def __init__(self, host_ip, update_interval=1.0, msg_format=HargassnerMessageTemplates.NANO_V14L):
        ...
        if update_interval < 0.5: update_interval = 0.5  # Hargassner sends 2 messages per second, no need to poll more frequent than that
        self._scheduler.add_job(lambda: self._update(), 'interval', seconds=update_interval)
        self._scheduler.start()

    def set_message_format(self, msg_format):
        if msg_format in HargassnerMessageTemplates.DICT:
            msg_format = HargassnerMessageTemplates.DICT[
                msg_format]  # if one of the constants has been passed, expand to full format string
        if not msg_format.startswith("<DAQPRJ>"):
            _LOGGER.error(f"Message template does not start with '<DAQPRJ>'.")
            return False
        self._paramData = {}
        root = xml.fromstring(msg_format)
        analog = root.find("ANALOG")
        for channel in analog.findall("CHANNEL"):
            unique_name = str(channel.get("name"))
            name_count = 1
            while unique_name in self._paramData:  # in case parameter name is duplicate, add a counter to make it unique
                name_count += 1
                unique_name = str(channel.get("name")) + "_" + str(name_count)
            self._paramData[unique_name] = HargassnerAnalogueParameter(unique_name, int(channel.get("id")),
                                                                       str(channel.get("unit")))
        ofs_digital = len(
            self._paramData)  # assuming that channel ids/indices are listed consecutively without any misses!
        len_digital = 0
        digital = root.find("DIGITAL")
        for channel in digital.findall("CHANNEL"):
            self._paramData[str(channel.get("name"))] = HargassnerDigitalParameter(str(channel.get("name")),
                                                                                   ofs_digital + int(channel.get("id")),
                                                                                   1 << int(channel.get("bit")))
            len_digital = int(channel.get("id")) + 1  # assuming that channel ids are increasing
        self._expectedMsgLength = ofs_digital + len_digital
    ...
    def _update(self):
        if self._updateTimeoutUntil >= int(time.time()):
            return
        ...
        if self._connectionOK:
            ...
            if last_msg_start < 0:
                ...
                log_msg = f"Received message contains no data. ({str(self._emptyMessages)})"
            ...
            if len(msg) != self._expectedMsgLength:
                self._malformedMessages += 1

                log_msg = f"Received message has unexpected length. (expected: {str(self._expectedMsgLength)} got:" \
                          f" {str(len(msg))}) ({self._malformedMessages})"
                if self._malformedMessages >= 3:
            ...
            for param in self._paramData.values():
                param.initialize_from_message(msg)
    ...
    def get_value(self, param_name):
        param = self._paramData.get(param_name)
        if param is None:
            _LOGGER.error(f"Parameter key {param_name} not known.")
            return None
        return param.value()
